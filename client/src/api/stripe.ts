...
export const createTopUpPaymentIntent = async (data: { packageId: string }) => {
  try {
    const response = await api.post('/api/stripe/create-topup-payment-intent', data);
    return response.data;
  } catch (error) {
...
export const setDefaultPaymentMethod = async (data: { paymentMethodId: string }) => {
  try {
    const response = await api.post('/api/stripe/set-default-payment-method', data);
    return response.data;
  } catch (error) {
...
export const deletePaymentMethod = async (paymentMethodId: string) => {
  try {
    const response = await api.delete(`/api/stripe/payment-methods/${paymentMethodId}`);
    return response.data;
  } catch (error) {
