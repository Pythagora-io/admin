...
router.get('/', requireUser, async (req, res) => {
  try {
    ...
    if (!['drafts', 'deployed'].includes(type)) {
      return res.status(400).json({ error: 'Invalid project type. Must be either "drafts" or "deployed"' });
    }
...
router.delete('/', requireUser, async (req, res) => {
  try {
    ...
    const result = await ProjectService.deleteProjects(req.user._id, projectIds);
...
router.put('/:id', requireUser, async (req, res) => {
  try {
    ...
    const updatedProject = await ProjectService.updateProject(id, req.user._id, updateData);
...
router.put('/:id/rename', requireUser, async (req, res) => {
  try {
    ...
    const updatedProject = await ProjectService.updateProject(id, req.user._id, { title });
...
router.post('/:id/duplicate', requireUser, async (req, res) => {
  try {
    ...
    console.log(`Attempting to duplicate project ${id} for user ${req.user._id}`);
    ...
    const duplicateProject = await ProjectService.createProject(req.user._id, duplicateData);
...
router.post('/:id/deploy', requireUser, async (req, res) => {
  try {
    ...
    const deployedProject = await ProjectService.deployProject(id, req.user._id);
...
router.put('/:id/access', requireUser, async (req, res) => {
  try {
    ...
    // Check if the user is the owner of the project
    if (project.userId.toString() !== req.user._id.toString()) {
      return res.status(403).json({ error: 'Unauthorized to update project access' });
    }
