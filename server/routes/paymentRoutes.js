...
router.get("/", requireUser, async (req, res) => {
  try {
    ...
    if (payments.length === 0) {
      console.log(
        `No payments found, attempting to sync with Stripe for user ID: ${req.user._id}`,
      );
      const user = await UserService.get(req.user._id);

      if (user.stripeCustomerId) {
        console.log(
          `Found Stripe customer ID: ${user.stripeCustomerId}, syncing payments`,
        );
        payments = await PaymentService.syncStripePayments(
          req.user._id,
...
router.post("/sync", requireUser, async (req, res) => {
  try {
    ...
    const payments = await PaymentService.syncStripePayments(
      user._id,
      user.stripeCustomerId,
    );
